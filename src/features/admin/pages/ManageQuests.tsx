...
// ⭐️ 1. Import Type และ ข้อมูล Mock มาจากไฟล์ส่วนกลาง
// (แก้ Path "@/data/mockData" ให้ตรงกับไฟล์จริงของคุณนะครับ)
import { Quest, mockQuests } from "@/data/mockData";

const ManageQuest = () => {
  const navigate = useNavigate();

  // ⭐️ 2. โหลดข้อมูล mockQuests เข้าไปใน State
  const [quests, setQuests] = useState<Quest[]>(mockQuests);
  
  // State สำหรับจัดการ Modal การแก้ไข
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [currentEdit, setCurrentEdit] = useState<Quest | null>(null);
  ...
  const handleSaveEdit = (e: React.FormEvent) => {
    ...
    setQuests(
      quests.map((q) =>
        q.id === currentEdit.id ? currentEdit : q
      )
    );
            ...
            {quests.length === 0 ? (
              ...
              quests.map((quest) => (
                <tr key={quest.id} className="border-b border-[#333]/30 hover:bg-white/5 transition-colors">
                  <td className="p-3 text-muted-foreground text-xs">{quest.id}</td>
                  <td className="p-3">
                    ...
                    <div className="text-[10px] text-muted-foreground mt-1 truncate max-w-[300px]">
                      {quest.category} • {quest.estimatedTime}
                    </div>
                  </td>
                  <td className="p-3 text-muted-foreground text-sm">{quest.providerName}</td>
                  <td className="p-3 text-center text-yellow-400 font-bold text-sm">{quest.rewardPoints} pts</td>
                  <td className="p-3 text-center text-accent text-sm">Lv.{quest.difficulty}</td>
          ...
          <PixelFrame variant="dark" className="relative p-8 w-full max-w-2xl my-8">
            ...
            <form onSubmit={handleSaveEdit} className="space-y-4">
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  <label className="block text-[10px] text-muted-foreground mb-2 uppercase">Provider Name</label>
                  <PixelInput
                    type="text"
                    value={currentEdit.providerName}
                    onChange={(e) => setCurrentEdit({ ...currentEdit, providerName: e.target.value })}
                    required
                  />
                </div>
              </div>
              ...
              <div className="flex gap-4">
                ...
                <div className="w-24">
                  <label className="block text-[10px] text-muted-foreground mb-2 uppercase">Difficulty</label>
                  <PixelInput
                    type="number"
                    min={1}
                    max={5}
                    value={currentEdit.difficulty}
                    onChange={(e) => setCurrentEdit({ ...currentEdit, difficulty: Number(e.target.value) })}
                    required
                  />
                </div>
                <div className="flex-1">
                  ...
                  <select
                    ...
                    onChange={(e) => setCurrentEdit({ ...currentEdit, status: e.target.value as Quest["status"] })}
              ...
              <div>
                <label className="block text-[10px] text-muted-foreground mb-2 uppercase">Short Description</label>
                <input
                  type="text"
