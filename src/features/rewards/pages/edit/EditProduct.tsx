    ...
    useEffect(() => {
      if (product) {
        ...
        if (product.imageUrl) {
          ...
          const fileNameFromUrl = product.imageUrl.split('/').pop()?.split('?')[0];
          setFileName(fileNameFromUrl || "current_image");
        }
      }
    }, [product]);
      ...
      <div className={`max-w-[700px] mx-auto px-4 py-8 ${i18n.language === "th" ? "font-['TA_8bit']" : ""}`}>
        ...
        <PixelFrame>
          <h1 className={`flex items-center gap-2 font-pixel pixel-text-shadow mb-2 ${fontClass}`}>
            <PixelStore size={24} className="text-yellow-500" /> {t("editProduct.title")}
          </h1>
