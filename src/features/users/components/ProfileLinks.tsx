  ...
  return (
    <>
      <PixelFrame className="mb-6">
        ...
        {githubUrl ? (
          <a
            ...
            className={`text-foreground hover:text-gray-300 font-pixel ${fontClass} transition-colors break-all`}
          >
            {githubUrl}
          </a>
        ) : (
      ...
      <PixelFrame className="mb-6">
        ...
        >
          <Gitlab size={20} className="text-yellow-400 shrink-0" />
          {t("userProfile.gitlabLinks")}
        </h2>
        {gitlabUsername ? (
          <span className={`text-foreground font-pixel ${fontClass}`}>
            {gitlabUsername}
          </span>
        ) : (
          <p className={`text-muted-foreground font-pixel ${fontClass}`}>
            {t("userProfile.noGitlab")}
          </p>
        )}
      </PixelFrame>
