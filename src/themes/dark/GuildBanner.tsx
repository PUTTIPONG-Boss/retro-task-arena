...
const GuildBannerDark = () => {
  ...
  const styleTag = `
    ...
    .dk-banner-content { position: relative; z-index: 10; }
    .dk-banner-svg { position: absolute; inset: 0; width: 100%; height: 100%; z-index: 0; object-fit: cover; }
    .dk-pixel-corner { font-size: 0.4em; color: ${colors.textAccent}aa; position: absolute; opacity: 0.5; z-index: 20; }
    .dk-pixel-star { font-size: 0.6em; margin: 0 4px; }
    .dk-deco-line { font-size: 0.5em; color: ${colors.textMuted}; }
    .dk-deco-icon { font-size: 0.8em; color: ${colors.textMuted}; }
    .dk-pixel-border-top, .dk-pixel-border-bottom {
  ...
  return (
    <>
      ...
      <div className={`dk-retro-banner p-10 flex flex-col items-center justify-center gap-4 ${fontClass}`}>
        <svg className="dk-banner-svg" viewBox="0 0 1600 900" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
          <defs>
            <linearGradient id="dk_banner_baseBg" x1="800" y1="0" x2="800" y2="900" gradientUnits="userSpaceOnUse">
              <stop offset="0" stopColor="#1A1A1A"/>
              <stop offset="1" stopColor="#0A0A0A"/>
            </linearGradient>
            <radialGradient id="dk_banner_centerSpot" cx="0.5" cy="0.5" r="0.6">
            ...
            <pattern id="dk_banner_dither" width="8" height="8" patternUnits="userSpaceOnUse">
            ...
            <pattern id="dk_banner_pixelWall" width="128" height="64" patternUnits="userSpaceOnUse">
          ...
          <rect width="1600" height="900" fill="url(#dk_banner_baseBg)"/>
          <rect width="1600" height="900" fill="url(#dk_banner_pixelWall)"/>
          <rect width="1600" height="900" fill="url(#dk_banner_dither)"/>
          <rect width="1600" height="900" fill="url(#dk_banner_centerSpot)"/>
        ...
        <div className="dk-banner-content flex flex-col items-center justify-center gap-4">
          ...
          <div className="dk-pixel-corner bottom-2 right-3">─┘</div>

          {particles.map((p) => (
            <motion.span
              key={p.id}
              className="dk-pixel-coin absolute"
              style={{ left: p.left, bottom: "10%", fontSize: `${p.size}em`, color: colors.textAccent, pointerEvents: "none" }}
              animate={{ y: [0, -100, -150], x: [0, Math.random() * 20 - 10, 0], opacity: [0, 0.8, 0], scale: [0.3, 1, 0.2], rotate: [0, p.rotation, p.rotation * 2] }}
              transition={{ duration: p.duration, delay: p.delay, repeat: Infinity, ease: "easeOut" }}
            >●</motion.span>
          ))}

          <div className="flex items-center gap-3">
            <span className="dk-deco-line">━━━</span>
            <span className="dk-deco-icon">⚜</span>
            <span className="dk-deco-line">━━━</span>
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <motion.h1 className="dk-title-glow font-bold tracking-wide whitespace-nowrap" style={{ color: colors.textAccent, fontSize: '2em' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.5 }}>
              {t("questBoard.title")}
            </motion.h1>
            <motion.span animate={{ scale: [1, 1.2, 1], rotate: [0, -10, 10, 0] }} transition={{ duration: 1, delay: 1, repeat: Infinity, repeatType: "mirror" }}>
          ...
          <div className="dk-scrolling-text-container">
            <span className="dk-scrolling-text subtitle-text uppercase whitespace-nowrap" style={{ color: colors.textMuted }}>
          ...
          <div className="flex gap-2">
            {[0, 1, 2, 3, 4].map((i) => (
              <motion.span key={i} className="dk-pixel-star" style={{ color: colors.textAccent }} animate={{ opacity: [0.3, 1, 0.3], scale: [0.8, 1.2, 0.8] }} transition={{ duration: 1.5, delay: i * 0.25, repeat: Infinity, ease: "easeInOut" }}>
                ✦
              </motion.span>
            ))}
          </div>
...
export default GuildBannerDark;
